...
# Если порт 3000 занят (сервер уже запущен) — просто открыть браузер (игра + админка)
if lsof -ti:3000 >/dev/null 2>&1; then
  echo "ℹ️  Сервер уже запущен. Открываю игру и админку."
...
echo "   Откроются игра и админка. Это окно не закрывай — оно держит сервер."
echo "   Остановить: закрой окно или Ctrl+C."
echo ""

# Открыть браузер чуть позже, когда сервер поднимется: игра + редактор-админка
( sleep 1.5; open "http://localhost:3000"; sleep 0.4; open "http://localhost:3000/admin" ) &
